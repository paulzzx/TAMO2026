...
class StructProbeDataset(Dataset):
    def __init__(self, type, prompt_type='tablellama'):
        ...
        self.path = f'{global_path}/dataset/structProbe'
        self.datas = datasets.load_from_disk(self.path + '/structProbe')
...
class StructProbeDatasetPermute(Dataset):
    def __init__(self, type, prompt_type='tablellama'):
        super().__init__()
        self.type = type
        self.prompt_type = prompt_type

        self.path = f'{global_path}/dataset/structProbe_permute'
        self.datas = datasets.load_from_disk(self.path + '/structProbe')
        self.instruction = 'This is a table question answer task. The goal of this task is to answer the question given the table.'
        self._get_linear_table()
    ...
    def _get_linear_table(self, ):
        ...
        for data in self.datas[self.type]:
            ...
            for idx, row in enumerate(table_array[1:]):
                linear_table += ' row ' + row[0] + ' : ' + ' | '.join(row[1:])
            self.input_seg.append(linear_table)
