...
class TableHypergraphLLM(torch.nn.Module):
    ...
    def visualize_input_token_importance(self, sample):
        ...
        if self.dataset_name == 'fetaqa':
            input_ids = description.input_ids[:self.max_txt_len] + question.input_ids[:fetaqa_queation_len] + eos_user_tokens.input_ids + label_input_ids
        else:
            input_ids = description.input_ids[:self.max_txt_len] + question.input_ids + eos_user_tokens.input_ids + label_input_ids
        inputs_embeds = self.word_embedding(torch.tensor(input_ids).to(self.model.device))
        ...
        return normalized_importance, bos_embeds.shape[0]
